...
def trim_silence(wav):
    ...
    Useful for M-AILABS dataset if we choose to trim the extra 0.5 silence
    at beginning and end. Thanks @begeekmyfriend and @lautjy for pointing
    out the params contradiction. These params are separate and tunable
    per dataset.
    """
...
def melspectrogram(wav, sample_rate, fft_size, hop_size, n_mels):
    D = _stft(preemphasis(wav, hparams.preemphasis, hparams.preemphasize),
              fft_size, hop_size)
    S = _amp_to_db(_linear_to_mel(np.abs(D), sample_rate,
                                  fft_size, n_mels)) - hparams.ref_level_db
...
def inv_linear_spectrogram(linear_spectrogram, fft_size, hop_size):
    ...
    if hparams.use_lws:
        ...
        return inv_preemphasis(_griffin_lim(S ** hparams.power, fft_size, hop_size), # noqa
                               hparams.preemphasis, hparams.preemphasize)


def inv_mel_spectrogram(mel_spectrogram, sample_rate,
                        fft_size, hop_size, n_mel):
...
def _mel_to_linear(mel_spectrogram, sample_rate, fft_size, n_mels,
                   _inv_mel_basis=None):
    # global _inv_mel_basis
    if _inv_mel_basis is None:
        _inv_mel_basis = np.linalg.pinv(
            _build_mel_basis(sample_rate, fft_size, n_mels))
...
def _normalize(S):
    if hparams.allow_clipping_in_normalization:
        if hparams.symmetric_mels:
            return np.clip((2 * hparams.max_abs_value) * ((S - hparams.min_level_db) / (-hparams.min_level_db)) - hparams.max_abs_value, # noqa
                           -hparams.max_abs_value, hparams.max_abs_value)
        else:
            return np.clip(hparams.max_abs_value * ((S - hparams.min_level_db) / (-hparams.min_level_db)), 0, hparams.max_abs_value) # noqa

    assert S.max() <= 0 and S.min() - hparams.min_level_db >= 0
    if hparams.symmetric_mels:
        return (2 * hparams.max_abs_value) * ((S - hparams.min_level_db) / (-hparams.min_level_db)) - hparams.max_abs_value # noqa
    else:
        return hparams.max_abs_value * ((S - hparams.min_level_db) / (-hparams.min_level_db)) # noqa
    ...
    if hparams.allow_clipping_in_normalization:
        if hparams.symmetric_mels:
            return (((np.clip(D, -hparams.max_abs_value,
                              hparams.max_abs_value) + hparams.max_abs_value) * -hparams.min_level_db / (2 * hparams.max_abs_value)) + hparams.min_level_db) # noqa
        else:
            return ((np.clip(D, 0, hparams.max_abs_value) * -hparams.min_level_db / hparams.max_abs_value) + hparams.min_level_db) # noqa

    if hparams.symmetric_mels:
        return (((D + hparams.max_abs_value) * -hparams.min_level_db / (2 * hparams.max_abs_value)) + hparams.min_level_db) # noqa
    else:
        return ((D * -hparams.min_level_db / hparams.max_abs_value) + hparams.min_level_db) # noqa
...
if __name__ == "__main__":
    ...
    mels = np.load(os.path.join(out_dir, mel_filename))
    mels = mels.T
    print(np.array(mels).shape)
    # print it's values
    assert np.array(mels).shape[0] == recon_n_mels

    signal_recons = inv_mel_spectrogram(melspectrogram,
                                        recon_sample_rate,
                                        recon_fft_size,
                                        recon_hop_size,
                                        recon_n_mels)

    save_wav(signal, "./recon-ljspeech-audio\
                      -00001-{}-{}-{}-{}.wav".format(recon_sample_rate,
                                                     recon_fft_size,
                                                     recon_hop_size,
                                                     recon_n_mels))
